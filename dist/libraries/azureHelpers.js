 ...
 */
export const submitQuestionGeneralGPT = async (question, system, base, apiKey, model = "bmi-centsbot-pilot") => {
 ...
 */
export const submitQuestionDocuments = async (question, system, base, apiKey, searchEndpoint, searchKey, indexName, model = "gpt-4o", searchType = "semantic", // Default to semantic search
searchConfig = {}) => {
    try {
        ...
        if (!searchResults.success) {
            ...
            throw new Error(`Failed to retrieve relevant documents: ${searchResults.error}`);
        }
        // Log search results count
        console.log(`Found ${searchResults.results?.length || 0} relevant documents`);
        ...
        if (searchResults.results && searchResults.results.length > 0) {
            ...
            context = searchResults.results
                .map((doc, index) => {
                const score = doc["@search.score"] || doc["@search.rerankerScore"] || "N/A";
        ...
        const response = await submitQuestionGeneralGPT(promptWithContext, system, base, apiKey, model);
        if (!response.success) {
 ...
 */
export const querySearchIndex = async ({ searchEndpoint, searchKey, indexName, queryText, searchType = "vector", top = 5, select = ["content", "title"], semanticConfiguration = "", filter = undefined, orderBy = undefined, includeTotalCount = false, }) => {
    try {
        ...
        Object.keys(searchRequest).forEach((key) => (searchRequest[key] === undefined || searchRequest[key] === null) &&
...
export const generateEmbedding = async (text) => {
    ...
    const url = "https://azopenai-pilot.openai.azure.com/openai/deployments/text-embedding-ada-002/embeddings?api-version=2023-05-15";
    const response = await fetch(url, {
        method: "POST",
        headers: {
            "Content-Type": "application/json",
